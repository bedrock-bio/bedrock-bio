...
test_that("tables field is character vector of fully-qualified names", {
  ...
  expect_true(all(startsWith(result$tables, "dbsnp.")))
  expect_true("dbsnp.vcf" %in% result$tables)
})

test_that("tables list agrees with list_tables filtered to namespace", {
  result <- describe_namespace("dbsnp")
  from_list <- list_tables()[startsWith(list_tables(), "dbsnp.")]
